...
export function initQuickLinks({ root } = {}) {
  ...
  function showContextMenu(e, site) {
    ...
    const menuItems = [
      ...
      {
        text: chrome.i18n.getMessage("createQRCode"), icon: 'qr_code', action: () => {
          window.dispatchEvent(new CustomEvent('ntm:create-qr-code', {
            detail: { url: site.url, title: site.name }
          }));
        }
      }
    ];
  ...
  // createQRCode removed: now handled by React QRCodeDialogPortal.jsx via ntm:create-qr-code event
